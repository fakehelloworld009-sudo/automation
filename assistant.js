...
async function preventElementHiding(page) {
    ...
    await page.addInitScript(() => {
        ...
        const originalDocReset = document.reset;
    ...
    await page.addInitScript(() => {
        ...
        function storeCheckboxes() {
            ...
            window.__PROTECTED_ELEMENTS__.checkboxes = checkboxes.map(cb => ({
                id: cb.id,
                name: cb.name,
                value: cb.value,
                checked: cb.checked,
        ...
        function storeOptions() {
            ...
            window.__PROTECTED_ELEMENTS__.options = options.map(opt => ({
                ...
                if (mutation.type === 'childList') {
                    ...
                    mutation.removedNodes.forEach(node => {
                        if (node.tagName === 'INPUT' ||
                            ...
                            node.className?.includes?.('option')) {
        ...
        setInterval(() => {
            ...
            for (const custom of Array.from(customSelects)) {
                ...
                for (const opt of Array.from(options)) {
                    opt.style.cssText = 'display: block !important; visibility: visible !important; opacity: 1 !important;';
                }
                // Remove hiding classes
 ...
 */
async function scrollAndHighlightElement(targetText, action = 'INTERACT') {
    if (!state.page || state.page.isClosed())
        return false;
    try {
 ...
 */
async function cleanupPageAfterStep() {
    if (!state.page || state.page.isClosed())
        return;
    try {
 ...
 */
async function enablePersistentScrolling() {
    if (!state.page || state.page.isClosed())
        return;
    try {
        ...
        for (const frame of frames) {
            try {
                await frame.evaluate(() => {
                    ...
                    if (!window.__scrollMonitorRunning) {
                        window.__scrollMonitorRunning = true;
...
async function handleDropdown(target, value) {
    ...
    try {
                ...
                for (let i = 0; i < 4; i++) {
                    ...
                    if (select) {
                        ...
                        if (select.id) {
                            ...
                            selectSelector = `select[name="${select.name}"]`;
                        }
                        else {
        ...
        if (selectorInfo?.found) {
            ...
            const valueSet = await state.page.evaluate((params) => {
 ...
 */
async function handleHierarchicalDropdown(target) {
    if (!state.page || state.page.isClosed())
        return false;
    ...
    try {
        ...
        const dropdownInfo = await state.page.evaluate((searchParam) => {
        ...
        if (!dropdownInfo) {
            ...
            const fallbackInfo = await state.page.evaluate((searchOption) => {
                const selects = document.querySelectorAll('select');
                for (let i = 0; i < selects.length; i++) {
                    const el = selects[i];
                    const options = [];
            ...
            if (fallbackInfo) {
                ...
                let optionToSelect = dropdownToUse.options.find((opt) => opt.textLower === optionText.toLowerCase() || opt.textLower.includes(optionText.toLowerCase()));
                if (!optionToSelect) {
        ...
        if (dropdownInfo.type === 'SELECT') {
            ...
            if (!optionToSelect) {
                ...
                log(`   💡 Available options: ${dropdownInfo.options.map((o) => o.text).join(', ')}`);
                return false;
            }
            log(`\n📍 STEP 3️⃣  - SELECTING OPTION USING PLAYWRIGHT API...`);
            log(`   Option value: "${optionToSelect.value}"`);
            ...
            try {
                ...
                if (selector !== 'select') {
                    const matchCount = await state.page.evaluate((sel) => {
                ...
                try {
                    const result = await state.page.evaluate((args) => {
                        const selects = document.querySelectorAll('select');
                        for (let i = 0; i < selects.length; i++) {
                            const sel = selects[i];
 ...
 */
async function ensurePageScrollable() {
    if (!state.page || state.page.isClosed())
        return;
    try {
        await state.page.evaluate(() => {
 ...
 */
async function scrollToElement(locator, action = 'scroll') {
    if (!state.page || state.page.isClosed())
        return false;
    try {
