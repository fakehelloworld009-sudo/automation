...
async function aggressivelyProtectFormElements(page: Page): Promise<void> {
    await page.addInitScript(() => {
        ...
        let protectedCheckboxes: WeakMap<any, boolean> = new WeakMap();
        let protectedSelects: WeakMap<any, boolean> = new WeakMap();
        ...
        Object.defineProperty(CSSStyleDeclarationProto, 'cssText', {
            ...
            set(value: string) {
                ...
                if (cssLower.includes('display:none') || cssLower.includes('visibility:hidden') || 
                    ...
                    try {
                        const el = (this as any).ownerElement || (this as any).element;
...
async function restoreScrollPosition(): Promise<void> {
    if (!state.page || state.page.isClosed() || !savedScrollPosition) return;
    
    try {
        await state.page.evaluate(({ x, y }: { x: number; y: number }) => {
            window.scrollTo(x, y);
        }, savedScrollPosition);
